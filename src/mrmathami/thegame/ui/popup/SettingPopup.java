...
public class SettingPopup extends AbstractPopup {
    public SettingPopup(long createdTick, double posX, double posY, double width, double height, StackPane stackPane) {
        ...
        PopupButton closeButton = new PopupButton(0, 0, 0,
                ...
                (posY + padding)/Config.TILE_SIZE,
                20, "\ueee4");
        getPopupComponents().add(closeButton);
        getPopupComponents().add(new PopupLabel(0,
                (Config.SCREEN_WIDTH/2.0)/Config.TILE_SIZE,
                (posY + 80)/Config.TILE_SIZE,
                42, Color.rgb(0, 0, 0), "SETTING"));

        getPopupComponents().add(new PopupLabel(0,
                (posX + width/3)/Config.TILE_SIZE,
                (posY + height/3)/Config.TILE_SIZE,
                30, Color.rgb(0, 0, 0), "Music"));
        PopupButton volumeDownButton = new PopupButton(0, 0, 0,
                (posX + width/3 + 150/2 + padding)/Config.TILE_SIZE,
                (posY + height/3 - 30)/Config.TILE_SIZE,
                20, "\uef99");
        getPopupComponents().add(volumeDownButton);
        PopupSlideBar popupSlideBar = new PopupSlideBar((posX + width/3 + 150/2 + padding + 30 + padding)/Config.TILE_SIZE,
                (posY + height/3 - 30)/Config.TILE_SIZE,
                300, 40, GameAudio.getInstance().getMainVolume()*100);
        getPopupComponents().add(popupSlideBar);
        PopupButton volumeUpButton = new PopupButton(0, 0, 0,
                (posX + width/3 + 150/2 + padding + 30 + padding + 300 + 30 + padding)/Config.TILE_SIZE,
                (posY + height/3 - 30)/Config.TILE_SIZE,
                20, "\uefc1");
