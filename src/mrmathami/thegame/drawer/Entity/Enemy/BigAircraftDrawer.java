...
public final class BigAircraftDrawer implements EntityDrawer {
    ...
    public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity,
        ...
        WritableImage aircraftFullShadow = new WritableImage(reader,
                (int)((((BigAircraft)entity).getGID() - 1) % maxTileWidth * Config.TILE_SIZE), (Math.round((((BigAircraft)entity).getGID() - 1) / maxTileWidth + 1) * (int)(Config.TILE_SIZE)),
                (int)(Config.TILE_SIZE), (int)(Config.TILE_SIZE));
        WritableImage aircraftFullImage = new WritableImage(reader,
                (int)((((BigAircraft)entity).getGID() - 1) % maxTileWidth * Config.TILE_SIZE),
                (int)(Math.round((((BigAircraft)entity).getGID() - 1) / maxTileWidth) * Config.TILE_SIZE),
        ...
        WritableImage aircraft = new WritableImage(aircraftFullImagePixelReader,
                (int)(Config.TILE_SIZE/2 - Config.BIG_AIRCRAFT_ENEMY_WIDTH/2),
                (int)(Config.TILE_SIZE/2 - Config.BIG_AIRCRAFT_ENEMY_HEIGHT/2),
                (int)(Config.BIG_AIRCRAFT_ENEMY_WIDTH),
                (int)(Config.BIG_AIRCRAFT_ENEMY_HEIGHT));
        WritableImage shadow = new WritableImage(aircraftFullShadowPixelReader,
                (int)(Config.TILE_SIZE/2 - Config.BIG_AIRCRAFT_ENEMY_WIDTH/2),
                (int)(Config.TILE_SIZE/2 - Config.BIG_AIRCRAFT_ENEMY_HEIGHT/2),
                (int)(Config.BIG_AIRCRAFT_ENEMY_WIDTH),
                (int)(Config.BIG_AIRCRAFT_ENEMY_HEIGHT));
        ...
        if (((BigAircraft)entity).getHealth() < Config.BIG_AIRCRAFT_ENEMY_HEALTH) {
            ...
            graphicsContext.fillRect(screenPosX, screenPosY - 5,
                    ((BigAircraft) entity).getHealth() * 1.0 / Config.BIG_AIRCRAFT_ENEMY_HEALTH * Config.BIG_AIRCRAFT_ENEMY_WIDTH,
                    7);
        }
    }
}
