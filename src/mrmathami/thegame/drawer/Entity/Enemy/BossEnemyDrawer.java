...
public class BossEnemyDrawer implements EntityDrawer {
    ...
    public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity,
        ...
        WritableImage newImage = new WritableImage(reader,
                (int)((bossEnemy.getGID() - 1) % maxTileWidth * Config.TILE_SIZE),
                (int)(Math.round((bossEnemy.getGID() - 1) / maxTileWidth) * Config.TILE_SIZE),
                (int)(Config.TILE_SIZE),
                (int)(Config.TILE_SIZE));
        reader = newImage.getPixelReader();
        WritableImage bossImage = new WritableImage(reader,
                (int)(Config.TILE_SIZE/2 - bossEnemy.getBossWidth()/2),
                (int)(Config.TILE_SIZE/2 - bossEnemy.getBossHeight()/2),
                (int)bossEnemy.getBossWidth(),
                (int)bossEnemy.getBossHeight());
        ...
        if (bossEnemy.getHealth() < bossEnemy.getMaxHealth()) {
            ...
            graphicsContext.fillRect(screenPosX, screenPosY - 5,
                    bossEnemy.getHealth() * 1.0 / bossEnemy.getMaxHealth() * bossEnemy.getBossWidth(), 7);
        }
    }
}
