...
public final class GrabEnemyDrawer implements EntityDrawer {
    ...
    public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity,
        ...
        WritableImage newImage = new WritableImage(reader,
                (int)((((GrabEnemy)entity).getGID() - 1) % maxTileWidth * Config.TILE_SIZE),
                (int)(Math.round((((GrabEnemy)entity).getGID() - 1) / maxTileWidth) * Config.TILE_SIZE),
                (int)(Config.TILE_SIZE),
                (int)(Config.TILE_SIZE));
        reader = newImage.getPixelReader();
        WritableImage pirateShip = new WritableImage(reader,
                (int)(Config.TILE_SIZE/2 - Config.GRAB_ENEMY_WIDTH/2),
                (int)(Config.TILE_SIZE/2 - Config.GRAB_ENEMY_HEIGHT/2),
                (int)Config.GRAB_ENEMY_WIDTH,
                (int)Config.GRAB_ENEMY_HEIGHT);
        ...
        if (((GrabEnemy)entity).getHealth() < Config.GRAB_ENEMY_HEALTH) {
            ...
            graphicsContext.fillRect(screenPosX, screenPosY - 5,
                    ((GrabEnemy) entity).getHealth() * 1.0 / Config.GRAB_ENEMY_HEALTH * Config.GRAB_ENEMY_WIDTH, 7);
        }
    }
}
