...
public final class TowerDestroyEffectDrawer implements EntityDrawer {
    ...
    public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity,
        ...
        WritableImage towerDestroyImage = new WritableImage(imgSheetPixelReader,
                (((TowerDestroyEffect)entity).getGID() - 1) % maxTileWidth * (int)screenWidth,
                (((TowerDestroyEffect)entity).getGID() - 1) / maxTileWidth * (int)screenHeight,
        ...
        for (int y = 0; y < towerDestroyImage.getHeight(); y++) {
            for (int x = 0; x < towerDestroyImage.getWidth(); x++) {
                if (!towerDestroyImagePixelReader.getColor(x, y).equals(Color.rgb(0, 0, 0, 0))) {
                    towerDestroyImagePixelWriter.setColor(x, y, Color.rgb((int) (towerDestroyImagePixelReader.getColor(x, y).getRed() * 255),
                            (int)(towerDestroyImagePixelReader.getColor(x, y).getGreen() * 255.0),
                            (int)(towerDestroyImagePixelReader.getColor(x, y).getBlue() * 255.0),
                            towerDestroyImagePixelReader.getColor(x, y).getOpacity()*((TowerDestroyEffect)entity).getBlur()));
                }
            }
        }
