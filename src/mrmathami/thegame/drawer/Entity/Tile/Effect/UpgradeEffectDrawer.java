...
public final class UpgradeEffectDrawer implements EntityDrawer {
    ...
    public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity, double screenPosX, double screenPosY, double screenWidth, double screenHeight, double zoom) throws FileNotFoundException {
        ...
        WritableImage upgradeImage = new WritableImage(imgSheetPixelReader, (((UpgradeEffect)entity).getGID() - 1) % maxTileWidth * (int)screenWidth, (((UpgradeEffect)entity).getGID() - 1) / maxTileWidth * (int)screenHeight, (int)screenWidth, (int)screenHeight);
        ...
        for (int y = 0; y < upgradeImage.getHeight(); y++) {
            for (int x = 0; x < upgradeImage.getWidth(); x++) {
                if (!upgradeImagePixelReader.getColor(x, y).equals(Color.rgb(0, 0, 0, 0))) {
                    upgradeImagePixelWriter.setColor(x, y, Color.rgb((int) (upgradeImagePixelReader.getColor(x, y).getRed() * 255), (int) (upgradeImagePixelReader.getColor(x, y).getGreen() * 255), (int) (upgradeImagePixelReader.getColor(x, y).getBlue() * 255), ((UpgradeEffect) entity).getBlur()));
                }
            }
        }
        ((UpgradeEffect)entity).setPosY(((UpgradeEffect)entity).getPosY() - 0.1);
