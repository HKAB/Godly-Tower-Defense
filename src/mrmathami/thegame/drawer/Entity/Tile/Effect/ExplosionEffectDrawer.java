...
public final class ExplosionEffectDrawer implements EntityDrawer {
    ...
    public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity,
        ...
        WritableImage explosionImage = new WritableImage(imgSheetPixelReader,
                (((ExplosionEffect)entity).getGID() - 1) % maxTileWidth * (int)screenWidth,
                (((ExplosionEffect)entity).getGID() - 1) / maxTileWidth * (int)screenHeight,
        ...
        for (int y = 0; y < explosionImage.getHeight(); y++) {
            for (int x = 0; x < explosionImage.getWidth(); x++) {
                if (!explosionImagePixelReader.getColor(x, y).equals(Color.rgb(0, 0, 0, 0))) {
                    explosionImagePixelWriter.setColor(x, y, Color.rgb((int) (explosionImagePixelReader.getColor(x, y).getRed() * 255),
                            (int)(explosionImagePixelReader.getColor(x, y).getGreen() * 255),
                            (int)(explosionImagePixelReader.getColor(x, y).getBlue() * 255),
                            ((ExplosionEffect) entity).getBlur()));
                }
            }
        }
