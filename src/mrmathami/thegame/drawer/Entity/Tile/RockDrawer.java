...
public final class RockDrawer implements EntityDrawer {
    ...
    public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity,
        ...
        WritableImage rockImage = new WritableImage(reader,
                (((Rock)entity).getGID() - 1) % maxTileWidth * (int)screenWidth,
                Math.round((((Rock)entity).getGID() - 1) / maxTileWidth) * (int)screenHeight,
                (int)screenWidth,
                (int)screenHeight);
