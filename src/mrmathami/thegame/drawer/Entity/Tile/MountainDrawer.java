...
public final class MountainDrawer implements EntityDrawer {
	...
	public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity,
		...
		WritableImage mountainImage = new WritableImage(reader,
				(((Mountain)entity).getGID() - 1) % maxTileWidth * (int)screenWidth,
				Math.round((((Mountain)entity).getGID() - 1) / maxTileWidth) * (int)screenHeight,
