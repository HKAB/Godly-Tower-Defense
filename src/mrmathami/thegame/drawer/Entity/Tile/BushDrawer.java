...
public final class BushDrawer implements EntityDrawer {
    ...
    public void draw(long tickCount, @Nonnull GraphicsContext graphicsContext, @Nonnull GameEntity entity,
        ...
        WritableImage bushImage = new WritableImage(reader,
                (((Bush)entity).getGID() - 1) % maxTileWidth * (int)screenWidth,
                Math.round((((Bush)entity).getGID() - 1) / maxTileWidth) * (int)screenHeight,
                (int)screenWidth,
                (int)screenHeight);
