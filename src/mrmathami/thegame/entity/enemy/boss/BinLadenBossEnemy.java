...
public class BinLadenBossEnemy extends BossEnemy {
    public BinLadenBossEnemy (long createdTick, double posX, double posY) {
        super(createdTick, posX, posY, Config.BIN_LADEN_BOSS_ENEMY_WIDTH, Config.BIN_LADEN_BOSS_ENEMY_HEIGHT,
                Config.BIN_LADEN_BOSS_ENEMY_HEALTH, Config.BIN_LADEN_BOSS_ENEMY_ARMOR, Config.BIN_LADEN_BOSS_ENEMY_SPEED,
                Config.BIN_LADEN_BOSS_ENEMY_REWARD, Config.BIN_LADEN_BOSS_ENEMY_GID);
    }
    ...
    public void skillCheck(@Nonnull GameField field) {
        if (this.isDestroyed()) {
            ...
            for (AbstractTower towerEntity:
                    GameEntities.getFilteredOverlappedEntities(field.getEntities(), AbstractTower.class, getPosX() - 1, getPosY() - 1, 3, 3)) {
